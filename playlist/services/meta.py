...
def genre(request, playlist_id):
	...
	# If no genre match...
		...
		new_genre = Genre(name=put['genre'])
		new_genre.save()
		playlist.genres.add(new_genre)
...
def add_subgenre(request, playlist_id):
	...
	try: 
		...
		playlist.subgenres.create(name=args['subgenre'], author=request.user)
		playlist.save()
		return success()

	if not (playlist.subgenre_set.filter(pk=subgenre.pk).exists()):
		playlist.subgenres.add(subgenre)
		playlist.save()
